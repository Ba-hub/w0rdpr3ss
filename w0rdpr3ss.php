	...
	function chota(){
	  ...
	  (  /  '@@@@@====@@@@@'  \  )
	   \(     /          \     )/
	     \   (            )   /
	          \          /
	    ✓w0rdpr3ss"."\033[0m".PHP_EOL;
	$param = $_SERVER["argv"][1];

	if($param == '-h'|| $param == '-help'|| $param == '--help' || $param == '--h'){
		echo "\033[1;33m To scan a target, Use: user$ ".$_SERVER["argv"][0]." https://www.wordpress-site.com"."\033[0m".PHP_EOL;
		die;
	}
	if(empty($param))
	{
	    echo "\033[1;35m Target URL is required! .. Please use: ".$_SERVER["argv"][0]." https://www.wordpress-site.com"."\033[0m".PHP_EOL;
	    die;
	}
		  
	if (filter_var($param, FILTER_VALIDATE_URL) === FALSE) {
		print('Not a valid URL .. please use the full format eg: https://www.wordpress-site.com')."\n";die;
	}
